...
def search_tables_for_match(tables: list[Table], search_val: str) -> Table:
    ...
    raise ValueError(f"The specified table ({search_val}) could not be found in your schema file.")
...
def _get_records(url: str, headers: dict[str, str], params: dict[str, Any], cur_records: Optional[list[dict[str, Any]]]=None) -> tuple[list[dict[str, Any]], str]:
...
def get_airtable_records(schema: dict[str, Any], table: str, fields: Optional[list[str]]=None, formula: str=None):
    ...
    for record in data:
        ...
        record_fields = {k: v[0] if isinstance(v, list) else v for k, v in record_fields.items()}       
